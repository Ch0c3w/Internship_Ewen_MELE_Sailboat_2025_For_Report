...
def display_scenario_logs(logs : np.ndarray, ref_point : np.ndarray, listscenario : list) -> None:
...
def split_logs(log_number : int) -> list:
    logs = read_log(log_number).T
    # print(f'logs : {logs}')
    split_logs = []
    current_scenario = logs[0][-1]
    current_mission_logs = []
    for log in logs:
        if log[-1] == 9:
            pass
        elif log[-1] == current_scenario:
            current_mission_logs.append(log.copy())
        else:
            current_scenario = log[-1]
            split_logs.append(current_mission_logs.copy())
            current_mission_logs = [log.copy()]
    split_logs.append(current_mission_logs.copy())
    result = []
    for tab in split_logs:
        result.append(np.array(tab).T)
    print(f'File n°{log_number} contains {len(result)} scenarios.')
    return result

def plot_logs_data(logs : np.ndarray, listscenario : list, ref_point : np.ndarray):
    ref_point_rad = ref_point * np.pi/180
    R = 6371e3
    lat = logs[1] * np.pi / 180
    long = logs[2] * np.pi / 180
    if logs[-1][0] != 9:
        listpoints_cart = []
        for coord in listscenario[int(logs[-1][0])]:
            listpoints_cart.append(GPS2cart(coord, ref_point))
        # print(listpoints_cart)
        listpoints_cart = np.array(listpoints_cart)
        plt.figure()
        plt.plot(listpoints_cart[:,0,0], listpoints_cart[:,1,0], color = 'black', label = "Wanted trajectory")
        x = R * (long - ref_point_rad[1]) * np.cos(ref_point_rad[0])
        y = R * (lat - ref_point_rad[0])
        plt.plot(x, y, color = 'blue', label = "Boat's trajectory")
        plt.scatter(x[0], y[0], color = 'green', label = 'Starting point', marker = '+', linewidths = 3)
        plt.scatter(x[-1], y[-1], color = 'red', label = 'End point', marker = '+', linewidths = 3)
        plt.legend()
        plt.title('Trajectory of the boat over time vs expected trajectory')
    t = (logs[0] - logs[0, 0]) / 1000
    # print(f"t = {t}")
    # print(f't shape = {t.shape}')
    plt.figure()
    plt.plot(t, logs[7], label = "Rudder command")
    plt.plot(t, logs[8], label = "Sail command")
    plt.legend()
    plt.title("Commands sent to the servomotors over time")
    plt.xlabel("Time (s)")
    plt.ylabel("Command (PWM value)")
    plt.figure()
    plt.plot(t, logs[3], label = "Yaw")
    plt.legend()
    plt.title("Evolution of boat's heading over time")
    plt.xlabel("Time (s)")
    plt.ylabel("Heading (°)")
    fig1, ax1 = plt.subplots()
    ax1.plot(t, logs[7], label = "Rudder command", color = 'orange')
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Command (PWM value)")
    ax2 = ax1.twinx()
    ax2.plot(t, logs[3], label = "Heading", color = 'blue')
    ...
    plt.legend()
    plt.xlabel("t (s)")
    plt.ylabel("Coordinate's value(m)")
    plt.title("Evolution of the boat cartesian coordinates over time")
    fig2, ax3 = plt.subplots()
    ax3.plot(t, logs[5], label = "Measured wind direction", color = 'blue')
    ax3.set_xlabel("Time (s)")
    ax3.set_ylabel("Measured wind's direction (°)")
    ax3.set_title('Evolution of the measured wind over time')
    ax4 = ax3.twinx()
    ax4.plot(t, logs[6], label = "Measured wind speed", color = 'orange')
    ax4.set_ylabel("Measured wind's speed (m/s)")
    fig2.legend()
    dir_wind = (logs[5] + logs[3]) * np.pi / 180
    head = logs[3] * np.pi / 180
    SOG = logs[4]
    true_wind_dir, true_wind_speed = [], []
    for i in range(len(dir_wind)):
        true_wind = np.array([[SOG[i] * np.sin(head[i]) - logs[6][i] * np.sin(dir_wind[i])],
                          [SOG[i] * np.cos(head[i]) - logs[6][i] * np.cos(dir_wind[i])]])
        true_wind_dir.append(sawtooth(np.arctan2(true_wind[0, 0], true_wind[1, 0]) - np.pi) * 180 / np.pi)
        true_wind_speed.append(np.linalg.norm(true_wind))
    fig3, ax5 = plt.subplots()
    ax5.plot(t, true_wind_dir, label="Computed true wind's direction", color='blue')
    ax5.set_xlabel("Time (s)")
    ax5.set_ylabel("Computed true wind's direction (°)")
    ax5.set_title('Evolution of the true wind over time')
    ax6 = ax5.twinx()
    ax6.plot(t, true_wind_speed, label="Computed true wind speed", color='orange')
    ax6.set_ylabel("Computed true wind's speed (m/s)")
    fig3.legend()
    plt.figure()
    plt.plot(t, true_wind_dir, label = "Computed true wind's direction")
    plt.plot(t, head * 180 / np.pi, label = 'Heading')
    plt.legend()
    plt.title("Comparison between the evolution of the boat's heading and the wind direction over time")
    plt.figure()
    plt.plot(t, SOG, label = 'Measured SOG')
    prev_time = logs[0][0]
    prev_x = x[0]
    prev_y = y[0]
    prev_SOG = 0
    ...
    for i in range(len(x)):
        if x[i] != prev_x or y[i] != prev_y:
            prev_SOG = np.linalg.norm(np.array([x[i] - prev_x, y[i] - prev_y])) / ((logs[0][i] - prev_time) / 1000)
            prev_x = x[i]
            prev_y = y[i]
            prev_time = logs[0][i]
        computed_SOG.append(prev_SOG)
    plt.plot(t, computed_SOG, label = 'Computed SOG')
    plt.xlabel('Time (s)')
    plt.ylabel('SOG (m/s)')
    plt.legend()
    plt.title('Comparison between the SOG measured by the GPS and the computed SOG over time')
    true_wind_dir, true_wind_speed = [], []
    for i in range(len(dir_wind)):
        true_wind = np.array([[computed_SOG[i] * np.sin(head[i]) - logs[6][i] * np.sin(dir_wind[i])],
                              [computed_SOG[i] * np.cos(head[i]) - logs[6][i] * np.cos(dir_wind[i])]])
        true_wind_dir.append(sawtooth(np.arctan2(true_wind[0, 0], true_wind[1, 0]) - np.pi) * 180 / np.pi)
        true_wind_speed.append(np.linalg.norm(true_wind))
    fig4, ax7 = plt.subplots()
    ax7.plot(t, true_wind_dir, label="Computed true wind's direction", color='blue')
    ax7.set_xlabel("Time (s)")
    ax7.set_ylabel("Computed true wind's direction (°)")
    ax7.set_title('Evolution of the true wind computed using computed SOG instead of measured one over time')
    ax8 = ax7.twinx()
    ax8.plot(t, true_wind_speed, label="Computed true wind speed", color='orange')
    ax8.set_ylabel("Computed true wind's speed (m/s)")
    fig3.legend()
    map = folium.Map(location = ref_point, zoom_start = 21, tiles = None)
    folium.TileLayer("Esri.WorldImagery").add_to(map)
    points_traj = list(zip(logs[1], logs[2]))
    points_path = listscenario[int(logs[-1][0])]
    folium.PolyLine(points_traj, color="purple", weight=3, opacity=0.8).add_to(map)
    folium.PolyLine(points_path, color="blue", weight=3, opacity=0.8).add_to(map)
    folium.CircleMarker(
        location=points_traj[0],
        radius=4,  # petite taille
        color='green',
        fill=True,
        fill_opacity=0.6,
        opacity=0.6,
...
if __name__ == '__main__' :
    ...
    # toKML(51)
    plot_logs_data(split_logs(51)[2], listscenario, water_refpoint)
    # display_scenario_logs(split_logs(51)[6], water_refpoint, listscenario)
